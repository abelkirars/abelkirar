...
vi.mock("server-only", () => ({}));
const m = vi.hoisted(() => ({ auth: vi.fn(), create: vi.fn(), configure: vi.fn(), open: vi.fn(), prepare: vi.fn(), enroll: vi.fn() }));
...
vi.mock("@/lib/courses/create-enrollment", () => ({ createEnrollmentAndInitialPayment: m.enroll }));
...
import { POST as enroll } from "../course-applications/[id]/enroll/route";
...
it("blocks anonymous create, schedule/open and preparation before resolving any IDs", async () => {
  ...
  expect((await enroll(request(), params)).status).toBe(401);
  for (const fn of [m.create, m.configure, m.open, m.prepare, m.enroll]) expect(fn).not.toHaveBeenCalled();
});
