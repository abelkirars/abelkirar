...
import { listAdminCoursePayments, PAYMENT_KIND_FILTERS, PAYMENT_QUEUE_FILTERS, paymentQueueFilter } from "@/lib/courses/admin-payments";
import { Container } from "@/components/marketing/container";

export const dynamic = "force-dynamic";
export default async function CoursePaymentsPage({ searchParams }: { searchParams: Promise<{ status?: string; kind?: string; page?: string }> }) {
  await requireAdminPage();
  const query = await searchParams;
  const status = paymentQueueFilter(query.status);
  const kind = PAYMENT_KIND_FILTERS.find(value => value === query.kind) ?? "ALL";
  const rawPage = Number(query.page || 1);
  const page = Number.isInteger(rawPage) && rawPage > 0 ? Math.min(rawPage, 10000) : 1;
  const { payments, count } = await listAdminCoursePayments(status, page, kind);
  return <Container className="max-w-6xl space-y-6 py-10">
    ...
    <nav aria-label="Payment status filters" className="flex flex-wrap gap-2">{PAYMENT_QUEUE_FILTERS.map(filter =>
      <Link key={filter} href={`/admin/course-payments?status=${filter}&kind=${kind}`} aria-current={filter === status ? "page" : undefined}
    ...
    <nav aria-label="Payment kind filters" className="flex flex-wrap gap-2">{PAYMENT_KIND_FILTERS.map(filter => <Link key={filter} href={`/admin/course-payments?status=${status}&kind=${filter}`} aria-current={filter === kind ? "page" : undefined} className={`rounded-lg border px-3 py-3 text-sm ${filter === kind ? "border-primary bg-secondary text-secondary-foreground" : "border-border bg-card"}`}>{filter.replaceAll("_", " ")}</Link>)}</nav>
    ...
    <div className="grid gap-4 md:grid-cols-2">{payments.map(p => <Link key={p.id} href={`/admin/course-payments/${p.id}`} className="space-y-2 rounded-xl border border-border bg-card p-5 hover:border-primary">
      ...
      {p.dueAt && <p className={p.status === "PAST_DUE" ? "font-semibold text-destructive" : "text-sm"}>{p.status === "PAST_DUE" ? "PAST DUE · " : "Due "}{p.dueAt.toISOString()}</p>}
    ...
    <nav className="flex gap-5" aria-label="Payment pages">
      {page > 1 && <Link href={`/admin/course-payments?status=${status}&kind=${kind}&page=${page - 1}`}>Previous</Link>}
      {page * 50 < count && <Link href={`/admin/course-payments?status=${status}&kind=${kind}&page=${page + 1}`}>Next</Link>}
    </nav>
  </Container>;
}
