...
from `messages/am.json` (Amharic) and `messages/en.json` (English source): **42 keys** — 41 as of the
initial extraction, plus `orderConfirmation.imageAttachFailed`, added when the Custom Made reference-photo
upload feature shipped (see `docs/DECISIONS.md`). Older entries in `docs/DECISIONS.md` that say "41" are
historical and describe the count at the time they were written, not a discrepancy.
...
27. `orderConfirmation.imageAttachFailed`
...
28. `paymentLabels.pendingQuote`
    EN: Awaiting quote

## validation (2 keys)

29. `validation.enterCustomOrderDescription`
    EN: Describe your custom order (at least 10 characters)
30. `validation.customOrderDescriptionTooLong`
    EN: Description must be 4000 characters or fewer

## emails (12 keys)

31. `emails.customOrderPending.subject`
    EN: Custom order request {orderNumber} received
32. `emails.customOrderPending.pricingNotice`
...
33. `emails.customOrderPending.greeting`
    EN: Thank you for your custom order request, {customerName}.
34. `emails.customOrderPending.orderNumberLabel`
    EN: Order number
35. `emails.customOrderPending.productLabel`
    EN: Instrument
36. `emails.customOrderPending.descriptionLabel`
    EN: Your request
37. `emails.quoteReady.subject`
    EN: Your quote for order {orderNumber} is ready
38. `emails.quoteReady.greeting`
    EN: Hi {customerName},
39. `emails.quoteReady.quoteBody`
    EN: Thank you for your patience. We've reviewed your custom order request (order {orderNumber}) and your final price is {total}.
40. `emails.quoteReady.totalLabel`
    EN: Total
41. `emails.quoteReady.paymentMethodLabel`
    EN: Payment method
42. `emails.quoteReady.submitConfirmationNotice`
...
7 of the 42 keys have one or more `{placeholder}` tokens — unchanged from the original list;
`imageAttachFailed` has none. The Amharic translation must reproduce the token(s) verbatim.
...
None. Checked all 42 English source strings above, including `imageAttachFailed`, for "Zelle", "Cash App",
or any other brand name — none of the currently-untranslated keys mention one. (For reference, elsewhere in
